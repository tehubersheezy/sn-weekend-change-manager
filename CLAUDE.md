...
- `npm run dev` — local dev server with LiveReload + `/api`,`/amb` proxies (Abey usually runs it himself; check ports 3000/8081 before starting another). Binds 3000 (ignores `PORT`; 8081 only ever happened as a fallback when 3000 was taken). It serves `dist/static` from its own watch build (dev-shaped: `main.js`, properly closed tags) and only re-builds on SOURCE changes. **`npm run build` overwrites `dist/static` with deploy-shaped output** (`main.jsdbx` + Jelly `$[...]` param + XML-serialized self-closing `<script/>` — an HTML parser swallows everything after it, so the dev page goes blank with zero console errors). After any build/deploy while the dev server runs, `touch` a client source file to re-trigger the watch build. Also: `/uxasset/externals` falls through to the instance proxy, so a missing local bundle silently serves the DEPLOYED code — check which `<script src>` the root page references before trusting what you see.
